...
import EasyBookingZerbitzaria.DL.pertsona;
			...
			try {
				...
				if (aukera == 1) {
					try {
						...
						erabiltzailea e1 = new erabiltzailea("73037449Y", "Eneko", "Atxa", 21,
								"eneko.atxa@opendeusto.es", "enekoatxa", "enekoatxa");
						erabiltzailea e2 = new erabiltzailea("34564587F", "Imanol", "Aizpuru", 22,
								"ima.aizpuru@opendeusto.es", "imanolaizpuru", "imanolaizpuru");
						erabiltzailea e3 = new erabiltzailea("98547567L", "Mikel", "Martinez", 21,
								"mikelmartinezz@opendeusto.es", "mikelmartinez", "mikelmartinez");
				...
				// 73037449Y nan dun erabiltzailea bilatu
					...
					try {
						...
						Query<erabiltzailea> erabiltzaileQuerya = persistentManager
								.newQuery("SELECT FROM " + erabiltzailea.class.getName() + " WHERE nan == '73037449Y'");
				...
				// 2 bidaiari baino gehiagoko erreserbak ezabatu, querya
				// erabiliz
				else if (aukera == 4) {
					try {
						transaction.begin();
						@SuppressWarnings("unchecked")
						Query<erreserba> erreserbaQuerya = persistentManager
								.newQuery("SELECT FROM " + erreserba.class.getName()
										+ " WHERE bidaiariKopurua > 2 ORDER BY bidaiariKopurua ASC");
						...
						for (erreserba e : erreserbaQuerya.executeList()) {
							System.out.println("- erreserba: " + e.getKodea() + " bidaiari kopurua: "
									+ e.getBidaiariKopurua() + " ezabatuko da.");
